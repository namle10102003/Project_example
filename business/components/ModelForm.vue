...
const removeItemAt = (key: any, index: number) => {
    if (current.value) {
        if (current.value[key] && current.value[key].length > index) {
            current.value[key].splice(index, 1); ;
        }
    }
};
...
const saveEntry = async () => {
    ...
    if (id) {
        ...
        .then((response: any) => {
            ...
            origin.value = _cloneDeep(data);
            current.value = _cloneDeep(data);
            navigateTo(route.fullPath.replace('/new', `/${data.id}`), { replace: true });
        })
        .catch((e: any) => {
            console.error(e);
            error.value = getErrorMessage(e, t('an_error_occurred'));
